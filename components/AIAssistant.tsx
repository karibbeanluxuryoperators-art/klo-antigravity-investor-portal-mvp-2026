 ...
 * guest. Qualification is still tracked in the background (so the AI knows
 * when to propose a price + handoff) but is NEVER shown in the chat UI.
 *
 * Internal qualification state is sent to the backend, persisted in
 * localStorage, and the broker sees it in /admin/leads/:id.
 */
...
// Internal tracking — NEVER shown in the UI
...
const AIAssistant: React.FC<AIAssistantProps> = ({ t, lang }) => {
  ...
  const handleSendText = useCallback(async (overrideInput?: string) => {
    ...
    try {
      ...
      // Update qualification silently (internal state, never shown)
