        ...
        <div className="w-80 md:w-[28rem] mb-6 overflow-hidden flex flex-col" style={{ borderRadius: '0px', border: '1px solid rgba(184,150,62,0.18)', background: '#0a1518', boxShadow: '0 0 0 1px rgba(255,255,255,0.04), 0 32px 80px rgba(0,0,0,0.80), 0 0 60px rgba(0,168,181,0.04)', animation: 'klo-slide-up 0.38s cubic-bezier(0.16,1,0.3,1) forwards' }}>
          ...
          <div style={{ padding: '1.25rem 1.5rem', background: 'rgba(26,46,53,0.60)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0,168,181,0.10)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '0px', overflow: 'hidden', border: '2px solid rgba(184,150,62,0.30)' }}>
                <img src="https://images.pexels.com/photos/1181519/pexels-photo-1181519.jpeg?auto=compress&cs=tinysrgb&w=150" alt="María" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
              <div>
          ...
          <div ref={scrollRef} style={{ flex: 1, minHeight: '320px', maxHeight: '380px', overflowY: 'auto', padding: '1.5rem', background: '#0a1518', display: 'flex', flexDirection: 'column', gap: '1.5rem' }} className="custom-scrollbar">
            ...
            {notified && (
              <div style={{ padding: '0.625rem 1rem', background: 'rgba(184,150,62,0.10)', border: '1px solid rgba(184,150,62,0.30)', textAlign: 'center' }}>
                <p style={{ fontFamily: '"Inter", sans-serif', fontSize: '10px', fontWeight: 500, color: 'rgba(184,150,62,0.85)', letterSpacing: '0.20em', textTransform: 'uppercase' }}>{lang === 'en' ? 'Our concierge team has been notified' : lang === 'pt' ? 'Nossa equipe concierge foi notificada' : 'Nuestro equipo concierge ha sido notificado'}</p>
              </div>
            )}
          </div>
          ...
          {turnsUsed > 0 && turnsUsed < 10 && (
            <div style={{ padding: '0.375rem 1.5rem', textAlign: 'center', background: '#0a1518', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
              <span style={{ fontFamily: '"Inter", sans-serif', fontSize: '8px', fontWeight: 500, color: 'rgba(244,239,230,0.25)', letterSpacing: '0.20em', textTransform: 'uppercase' }}>{turnsUsed}/10</span>
            </div>
          )}
          <div style={{ padding: '1.25rem 1.5rem', background: 'rgba(10,21,24,0.80)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendText()} placeholder={t('assistant.placeholder') || 'Escriba su mensaje...'} disabled={isLoading || turnsUsed >= 10} style={{ flex: 1, padding: '0.875rem 1.25rem', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0px', color: 'rgba(244,239,230,0.90)', fontFamily: '"Inter", sans-serif', fontWeight: 300, fontSize: '13px', outline: 'none', transition: 'border-color 0.3s ease' }} onFocus={(e) => { e.currentTarget.style.borderColor = 'rgba(0,168,181,0.40)'; }} onBlur={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; }} />
            <button onClick={() => handleSendText()} disabled={isLoading || !input.trim() || turnsUsed >= 10} style={{ width: '44px', height: '44px', flexShrink: 0, borderRadius: '0px', background: '#00a8b5', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: isLoading || !input.trim() || turnsUsed >= 10 ? 0.4 : 1, transition: 'opacity 0.2s ease, transform 0.2s ease', boxShadow: '0 4px 16px rgba(0,168,181,0.25)' }} onMouseEnter={(e) => { if (!isLoading && input.trim() && turnsUsed < 10) e.currentTarget.style.transform = 'scale(1.05)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; }}>
